...
\begin{exe}
  ...
  \ex\label{ex:onOccasion} I have seen them \choices{\p{on}\_occasion\\\p{from}\_time\_to\_time}. [contrast \emph{on \dots occasion}, \cref{ex:onXOccasion}]
\end{exe}
...
%His reaction was angry; His angry reaction: also Manner, though not prepositional
  ...
  \begin{itemize}
    ...
    \begin{exe}
      \ex John is \choices{\p{on} his back\\\p{on} antibiotics\\\p{on} the ventilator\\\p{in} pain\\\p{in} a coma\\\p{into} sports}. (\rf{Manner}{Locus})
    \end{exe}
...
\paragraph{\emph{answer}, \emph{respond}, etc.}
\begin{exe}
