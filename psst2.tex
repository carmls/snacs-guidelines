...
\begin{itemize}
  ...
  \begin{itemize}
    ...
    \item Reworded the definition of \psst{SocialRel}.
...
\end{exe}
\nss{rev:}The property may be a fact or detail conventionally associated with someone in life for which there is no more specific role:
\begin{exe}
...
\shortdef{Piece of \textbf{property} (something potentially with monetary value) that an animate party has on a permanent or temporary basis, or that is transferred between parties. The \psst{Possession} must be \emph{alienable}, i.e.~not a part or attribute of the \psst{Possessor}.} 
...
\begin{exe}
  ...
  \ex\label{ex:goods} Goods: \begin{xlist}
    \ex They spent \$500 \p{on} the bicycle. [also at \psst{Theme}] (\rf{Possession}{Theme})
...
\begin{exe}
  ...
  \begin{multicols}{2}
    ...
    \end{xlist}
  \end{multicols}}
  \ex the rivalry \p{between} the teams\nss{rev} (\rf{SocialRel}{Whole}) [see \cref{ex:betweenParties}]
