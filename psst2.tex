...
\begin{exe}
  ...
  \ex \begin{xlist}
      ...
      \ajb{
      Comapare \p{of} and \p{'s}:
      \ex \choices{the waves \p{'} approach}
      \ex a salesman \p{'s} \choices{death\\murder}}
    \end{xlist}
  \ex \begin{xlist}
...
\begin{exe}
  ...
  \ajb{
  Compare \p{of} and \p{'s}:
  \ex The students \p{'} anger (\rf{Experiencer}{Possessor})}
...
or \psst{Source} (\pex{obtain/hear \p{from}}; adnominal \p{of}\ajb{ or genitive marking \p{'s}, \p{my}, etc.} as in \pex{works \p{of} Shakespeare}). 
