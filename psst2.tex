...
\begin{itemize}
  ...
  \begin{itemize}
    ...
    \item Clarify that \psst{Experiencer} applies to bodily \emph{sensations} (not physical bodily changes), and add cognition examples.
    \item Changed the treatment of replacees from \psst{InsteadOf} to \psst{Co-Theme}.
    \item List PP idioms in the index.
    \item At \psst{Path}, clarified that motion events can be located as points (\psst{Locus}).
    \item Rectify inconsistent treatment of a family member in relation to the family (now always \psst{OrgRole}).
