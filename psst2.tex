...
\begin{itemize}
  ...
  Clarified \psst{Time} vs.~\psst{DeicticTime}; 
  \nss{TODO:}clarified \psst{Direction} vis-\`{a}-vis \psst{Source}, \psst{Goal}, and \psst{Path}
\end{itemize}
...
\begin{exe}
  ...
  to fall under spatial, temporal, causal, or other subtypes like \psst{Manner}.
\end{itemize}
...
\begin{exe}
  ...
  \ex We left the party \p{after} an hour. [an hour after it started]
\end{exe}
But see \psst{DeicticTime} for preposition senses that are inherently relative to `now' 
(speech time).
...
expressed with a measure of time that serves as an offset relative to the speech time specifically.}

\begin{exe}
  \ex I ate \choices{10~minutes \p{ago}\\a while \p{back}}.\nss{not sure if this use of `back' patterns more like `ago' or more like `before'.}
  \ex\label{ex:inAmbiguousTime} I will eat \p{in} 10~minutes.
    \begin{xlist}
      ...
      \ex\label{ex:inDeicticTime} {} [`10~minutes from now' reading]: \psst{DeicticTime}
    \end{xlist}
\end{exe}
There is another sense of \p{in} that marks an explicit reference \psst{Time} 
(\pex{\p{in} the morning}). 
The \psst{DeicticTime} subcategory allows us to distinguish the sense in \cref{ex:inDeicticTime}.
...
\begin{exe}
  \ex\label{ex:IntrRefTime} To beat the crowds at the game, I will arrive an hour \choices{\p{before}\\\p{beforehand}\\ \#\p{ago}}.
  \ex\label{ex:TimeDiff} I took a seat in the waiting room. \choices{\p{After}\\??\p{In}} 15 minutes, the doctor saw me for an hour.
\end{exe}
...
\begin{exe}
  ...
  \ex The cat is \choices{\p{on\_top\_of}\\\p{beside}\\\p{near}} the dog.
  \ex the wheels \p{on} the bus
...
In English, the prototypical \psst{Source} preposition is \p{from}:
\begin{exe}
  \ex\label{ex:catBox} The cat jumped \p{from} the box.
  \ex\label{ex:catLedge} The cat jumped \p{from} the ledge.
  \ex\label{ex:internet} I got it \p{from} the internet.
  \ex people \p{from} France
  \ex The temperature is rising \p{from} a low of 30 degrees.
  \ex I have arrived \p{from} work.
  \ex\label{ex:coma} She awoke \p{from} a coma.
\end{exe}
...
\begin{exe}
  ...
  \exp{ex:catLedge} The cat jumped \choices{\p{off\_of}\\\p{off}} the ledge. (\rf{Source}{Direction})
\end{exe}
...
is somewhat salient:
\begin{exe}
  \exp{ex:internet} I got it \p{off} the internet. (\psst{Source})
  \exp{ex:coma} She came \p{out\_of} a coma. (\psst{Source})
  \ex We are moving \p{off\_of} that strategy. (\rf{Source}{Direction})
\end{exe}
\nss{is this too fine a line?}
...
\shortdef{An intermediate space between an initial position and a final position.}
...
This is typically used for intransitive prepositions (sometimes classified 
as adverbs). There may or may not be an implicit \psst{Source} or \psst{Goal}:
\begin{exe}
  \ex The bird flew \choices{\p{up}\\\p{out}\\\p{away}}.
  \ex The price shot \p{up}.
\end{exe}

\nss{What about conative (kick \p{at} the wall): Goal or Direction?
Away from: Source or Direction? 
Inside?}
...
\ex  I will appoint him \choices{\p{as}\\\p{since}} he is most qualified for the job.
\end{exe}
