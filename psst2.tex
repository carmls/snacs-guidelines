...
\begin{exe}
  \ex \begin{xlist}
      \ex This place is great \p{for} ping-pong. [no commitment to evaluating the place in general]
...
when it marks a ritualized activity such as a meal or holiday/commemoration, 
provided that the main event is in preparation for the observance: 
\begin{exe}
  \ex \psst{Purpose}:
    \begin{xlist}
    ...
    \end{xlist}
\end{exe}
However, if the activity marked by \p{for} is interpreted as containing the main event, 
...
to which it may or may not belong. The category stands for its members 
or prototypes:
\begin{exe}
  \ex He is short \p{for} a basketball player. (\psst{ComparisonRef})
\end{exe}
