...
\begin{itemize}
  ...
  \begin{itemize}
    ...
    \item \textbf{Substantially revised \psst{Possessor} and \psst{Possession} to clarify their scope.}
    ...
    \item \textbf{For \psst{Possession} and \psst{Theme}, added discussion of transfer, goods, and services. 
    Goods and other transferred items are now \psst{Possession}, while services remain as \psst{Theme}.}
    \item At \psst{Cost}, clarified the explanation of when money should be treated like any other possession.
    ...
    \item Revised the definition of \psst{Means} and clarified its relationship to \psst{Manner}.
    ...
    \item At \psst{Path}, clarified that motion events can be located as points (\psst{Locus}), and relocated \cref{ex:pathmanner} from \psst{Manner}.
...
\begin{exe}
  \ex\begin{xlist}
    ...
    \ex They won \p{for} the third year in$_{\psst{Manner}}$\nss{rev: TODO} a row. (\psst{Frequency})
...
\begin{exe}
  ...
  \end{xlist}
\end{exe}
\item The \textbf{form or shape} that an entity takes, including language of communication within an information source:\nss{rev}
  \begin{exe}
