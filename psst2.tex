...
\makeindex[name=construals,title={Index of Construals}]
...
\newcommand{\rf}[2]{\psst{#1}$\leadsto$\psst{#2}\index[construals]{\protect\psst{#1}$\leadsto$\protect\psst{#2}}}
...
\begin{itemize}
  ...
  \begin{xexe}
    ...
    \ex Don't hesitate \p{in} sending us a donation.
    \ex The owner refused to budge \p{on} the price.
    ...
    \ex They have almost anything you could want \choices{\p{when\_it\_comes\_to}\\\p{in\_terms\_of}} spy and surviellance equipment .
  \end{xexe}
\end{itemize}
...
\begin{exe}
  \ex Don't compare me \p{with} my sister! (\rf{Comparison}{Accompanier})
\end{exe}
...
\begin{exe}
  ...
  \begin{xlist}
    \ex \emph{Analogy reading:} This looks similar to a Van Gogh painting. (\psst{ComparisonRef})
  ...
  \begin{xlist}
    \ex \dots he had drunk a gallon of helium. (\psst{ComparisonRef}: analogy reading more likely)
...
two things, but rather an open-ended description. \psst{Manner} is therefore used:
\begin{exe}
  \ex\psst{Manner}:\begin{xlist}
    \ex I know what Steve looks \p{like}. (I know how Steve looks.)
    \ex What does her hair look \p{like}? (How does her hair look?)
    \ex What is the party \p{like}? (How is the party?)
  \end{xlist}
\end{exe}
...
\begin{exe}
  ...
  \ex\begin{xlist}
    ...
    \ex my phone service \p{with} Verizon (\rf{OrgRole}{Accompanier})
  \end{xlist}
