...
\begin{itemize}
  ...
  \begin{itemize}
    ...
    \item Rewrote \cref{sec:genitives}: Genitives/Possessives and updated corresponding examples 
    to reflect a clarified policy on possessive construals.
    \item Added \cref{sec:passives}: Passives and updated corresponding examples.
...
\begin{exe}
  ...
  \end{xlist}
  \ex \rf{Originator}{Possessor}: \begin{xlist}
...
\begin{exe}
  \ex\rf{SocialRel}{Gestalt}:\begin{xlist}
    ...
    \ex Lord Voldemort\p{'s} grandfather
  \end{xlist}
  \ex\begin{xlist}
    \ex the hood \p{of} the car (\psst{Whole})
    \ex the car\p{'s} hood (\rf{Whole}{Gestalt})
  \end{xlist}
  \ex\begin{xlist}
    \ex the arrival \p{of} the queen (\psst{Theme})
    \ex the queen\p{'s} arrival (\rf{Theme}{Gestalt})
  \end{xlist}
  \ex These are children\p{'s} clothes. [clothes intended for use and possession by children] (\rf{Beneficiary}{Possessor})
  \ex Shakespeare\p{'s} works [items that the creator or transferer possesses (at least initially)] (\rf{Originator}{Possessor})
\end{exe}
