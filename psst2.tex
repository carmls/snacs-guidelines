...
\eml{nathan.schneider@georgetown.edu} \and
...
\begin{exe}
  \ex \begin{xlist}
    ...
    \ex	The 3 layers \p{of} the cake
    \ex	The 3 prongs \p{of} the strategy
    \ex 2 \p{of} my 5 daughters
...
Used to construe \psst{Locus} for geographic ``containers'':
\begin{exe}
...
\begin{exe}
  ...
  \ex	A bag('s worth) \p{of} chips (\rf{Quantity}{Stuff})
  \ex	\begin{xlist}
    \ex An order \p{of} nuns (\rf{OrgRole}{Stuff})
    \ex	A chamber group \choices{\p{of}\\\p{with}} 5 players (\rf{OrgRole}{Stuff})
  \end{xlist}
\end{exe}
...
\begin{exe}
  ...
  \ex I am the baby \p{in} the family. (\rf{OrgRole}{Locus})
\end{exe}
