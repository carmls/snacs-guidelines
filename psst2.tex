...
The v2.5 hierarchy is a tree with 48~labels.
They are organized into three major subhierarchies: 
\psst{Circumstance} (18~labels), \psst{Participant} (12~labels),
...
\begin{itemize}
  ...
  \begin{itemize}
    ...
    \item \textbf{\sst{InsteadOf} has been merged with \psst{ComparisonRef}.}
  ...
  \begin{itemize}
    ...
    \item Changed the treatment of replacees from \psst{InsteadOf} to \psst{Co-Theme}.
  ...
  \begin{itemize}
    ...
    \begin{itemize}
      ...
      (see \psst{Stuff}, \psst{Quantity}, \psst{Characteristic}, \sst{OrgRole}).
...
In reciprocal or cooperative activites, there are considered to be multiple
\psst{Agent}s unless one of the parties is portrayed as an \psst{Instrument}
or affectee.}
...
Many events involve multiple agentive participants,
...
See also: \psst{Org}, \psst{Originator}, \psst{Source}, \psst{Stimulus}, \psst{Accompanier}, \psst{SocialRel}

\begin{history}
In v1 and v2.0--2.4, \psst{Agent} had a subtype \sst{Co-Agent} for core secondary agents.
The two were merged in v2.5.
\end{history}
...
\begin{exe}
  \ex\begin{xlist}
    \ex You shouldn't confuse/associate Mozart \p{with} Rossini. (\psst{Theme})
    \ex We are ready/eligible/due \p{for} an upgrade. (\rf{Theme}{Purpose})
...
and criticism about someone who is absent is not yelling at them.})
  ...
  \ex I'll have to check \p{with} my supervisor. (\rf{Recipient}{Accompanier})
\end{exe}
...
multiple \psst{Agent}s:
\begin{exe}
  \ex She \choices{swapped stories\\chatted} \p{with} her friends. (\rf{Agent}{Accompanier})
\end{exe}
...
See also \psst{Accompanier}, \psst{Beneficiary}, \psst{Org}, \psst{OrgMember}, and \fullref{sec:genitives}.
...
The construal \rf{Agent}{Accompanier} is used
for a second agentive participant in an activity,
however central that participant is to the activity:
\begin{exe}
  \ex \rf{Agent}{Accompanier}:\begin{xlist}
    \ex Do you want to walk \p{with} me?
    \ex Do you want to talk \p{with} me? (\emph{Either of two readings:} `Should we have a conversation?', or `Do you want to join me in talking to a third party?')
    \ex I fought \p{with} them to reform the regulation. (\emph{Either of two readings:} `I fought against them', or `We fought together for a common goal.')
  \end{xlist}
\end{exe}
...
\begin{exe}
  \ex\label{ex:preferOver} I prefer this restaurant \p{over} that one. (\rf{ComparisonRef}{Locus})\\{} 
  [paraphrase: I like this restaurant better \p{than} that one.]
\end{exe}
This is closely related to the notion of an alternative as in \cref{ex:alternativeOver}.
...
See also \psst{Accompanier} and \psst{Theme}.
...
\p{fewer\_than}, \lex[at\_least]{\p{at}\_least}, and \lex[at\_most]{\p{at}\_most}.\footnote{These constructions are 
...
\begin{exe}
  \ex \begin{xlist}
      \ex\label{ex:workwithSR} I work \p{with} Michael. (\rf{SocialRel}{Accompanier})
      \ex Joan has a class \p{with} Miss Zarves. (\rf{SocialRel}{Accompanier})
    \end{xlist}
...
\begin{exe}
  \ex\label{ex:workwithCA} I was working \p{with} Michael after lunch. (\rf{Agent}{Accompanier})
\end{exe}
...
\textbf{For English prepositions and possessives, \psst{Experiencer}, \psst{Stimulus}, \psst{Originator}, \psst{Recipient}, \psst{SocialRel}, \psst{Org}, and \psst{OrgMember}
