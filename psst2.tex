...
%His reaction was angry; His angry reaction: also Manner, though not prepositional
  ...
  \begin{exe}
    \ex\label{ex:smellOf} \choices{Your father smells\\The soup tastes} \p{of} elderberries. (\rf{Manner}{Topic}) [also~\cref{ex:smellOfNotCmp}]
  \end{exe}
...
\begin{exe}
  ...
  \end{xlist}
  \ex\label{ex:worksGen} Shakespeare\p{'s} works (\rf{Originator}{Possessor}) [cf.~\cref{ex:worksOf,ex:worksBy}]
\end{exe}
...
\begin{exe}
  \ex\label{ex:smellOfNotCmp} \choices{Your father smells\\The soup tastes} \p{of} elderberries. (\rf{Manner}{Topic}) [also~\cref{ex:smellOf}]
\end{exe}
