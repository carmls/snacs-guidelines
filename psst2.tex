...
Prototypical prepositions include \p{from}, \p{off}, \p{off\_of}, and \p{out\_of}:
\begin{exe}
  \ex The cat jumped \choices{\p{off\_of}\\\p{off}\\\p{from}} the ledge.
  \ex I got it \choices{\p{from}\\\p{off}} the internet.
  \ex people \p{from} France
  \ex The temperature is rising \p{from} a low of 30 degrees.
  \ex I have arrived \p{from} work.
  \ex She \choices{awoke \p{from}\\came \p{out of}} a coma.
\end{exe}
