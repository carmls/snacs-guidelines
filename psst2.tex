...
\begin{itemize}
  ...
  \begin{itemize}
    ...
    \item \textbf{\sst{OrgRole} has been split into \psst{Org} and \psst{OrgMember}, reflecting
  ...
  \begin{itemize}
    ...
    \item Rectified inconsistent treatment of a family member in relation to the family (now always \psst{OrgRole}).
...
See also: \psst{OrgRole}, \psst{Originator}, \psst{Source}, \psst{Stimulus}, \psst{Accompanier}, \psst{SocialRel}
...
and criticism about someone who is absent is not yelling at them.})
  ...
  \ex I'll have to check \p{with} my supervisor. (\rf{Recipient}{Theme}\nss{or Recip/Acc?})
\end{exe}
...
See also \psst{Accompanier}, \psst{Beneficiary}, \psst{OrgRole}, and \fullref{sec:genitives}.
...
\begin{exe}
  \ex	A piano quintet is a chamber group \p{with} a piano (in it)\\ (\rf{OrgRole}{PartPortion}) [repeated at \psst{OrgRole}]
\end{exe}
...
\begin{exe}
  ...
  \ex \rf{OrgRole}{Stuff}:
  	\begin{xlist}
      \ex An order \p{of} nuns [repeated at \psst{OrgRole}]
      \ex	A chamber group \choices{\p{of}\\\p{with}} 5 players [repeated at \psst{OrgRole}]
    \end{xlist}
\end{exe}
...
\begin{exe}
  ...
  \end{xlist}
  \ex I always do business \p{with} this company. (\rf{OrgRole}{Accompanier})
  ...
  and the object of the preposition describes the full membership:
    ...
    \end{xlist}
  \ex\rf{OrgRole}{PartPortion} if the governor is an organizational collective noun 
...
\textbf{For English prepositions and possessives, \psst{Experiencer}, \psst{Stimulus}, \psst{Originator}, \psst{Recipient}, \psst{SocialRel}, and \psst{OrgRole} 
