...
\begin{itemize}
  ...
  \begin{itemize}
    \item Clarified \psst{Time} vs.~\psst{DeicticTime}
...
\begin{exe}
  \ex I ate \choices{10~minutes \p{ago}\\a while \p{back}}.\footnote{While 
...
\begin{exe}
  ...
  \ex\label{ex:TimeDiff} I took a seat in the waiting room. \choices{\p{After}\\??\p{In}} 15 minutes, the doctor saw me for an hour. (\psst{Time})
\end{exe}
...
A conventional way to express one's status as a student at some school is 
...
\begin{exe}
  ...
  \ajb{\ex \choices{the City Council\p{'s} decisive vote\\the decisive vote of the City Council (\rf{Agent}{Possessor})}
  \ex we needed \p{her} help
  }
\end{exe}
...
\begin{exe}
  ...
  \ajb{\ex \choices{The anger \p{of} the students\\The students\p{'} anger} (\rf{Experiencer}{Possessor})}
...
or \psst{Source} (\pex{obtain/hear \p{from}}; adnominal \p{of} as in \pex{works \p{of} Shakespeare}\ajb{possessives \p{'s}, \p{my}, etc.}). 
...
\begin{exe}
  ...
  \ajb{\ex \choices{the money \p{of} the rich\\the rich\p{'s} money}}
  
  \ajb{\ex \p{our} dog} % POSSESSIVE
\end{exe}
...
\begin{exe}
  ...
  \ex\rf{SocialRel}{Possessor} \begin{xlist}
      ...
      \ajb{
      \ex Joan is \choices{the sister \p{of} John\\John\p{'s} sister}.
      \ex Joan is \choices{a student \p{of} Miss Zarves\\Miss Zarves\p{'s} student}.
      \ex \choices{the family \p{of} Miss Zarves\\Miss Zarves\p{'s} family}  
      }
    \end{xlist}
...
\begin{exe}
  ...
  \begin{xlist}
      ...
      \ajb{\choices{\ex the chairman \p{of} the board\\the board\p{'s} chairman}
      \choices{\ex the president \p{of} the U.S.\\the U.S.\p{'} president}
      \ex I am \choices{a loyal customer \p{of} Graeter's.\\Graeter\p{'s} loyal customer}
      \choices{\ex employees \p{of} Grunnings\\Grunnings\p{'} employees}
